...
class AgentState(TypedDict):
    ...
    retrieved_query_examples: str
    query_rag_time: float
...
class Config:
    ...
    enable_query_rag: bool
    bag_of_queries_path: str
    query_rag_index_dir: str
    query_rag_top_k: int
    query_rag_embedding_model: str
...
def load_runtime_resources(cfg: Config) -> RuntimeResources:
    query_rag_store = None
    if cfg.enable_query_rag:
        query_rag_store = build_or_load_query_rag(
            bag_of_queries_path=cfg.bag_of_queries_path,
            index_dir=cfg.query_rag_index_dir,
            embedding_model_name=cfg.query_rag_embedding_model,
        )
    ...
    return RuntimeResources(
        ...
        query_rag_store=query_rag_store,
    )
...
def retrieved_query_examples_message(examples_text: str) -> List[AnyMessage]:
    if not examples_text:
        return []
    return [SystemMessage(f"RETRIEVED_QUERY_TEMPLATES:\n{examples_text}")]
...
def build_generate_messages(state: AgentState, resources: RuntimeResources) -> List[AnyMessage]:
    ...
    msgs.extend(retrieved_query_examples_message(state.get("retrieved_query_examples", "")))
...
def make_graph(cfg: Config, resources: RuntimeResources):
    ...
    def node_prepare_context(state: AgentState) -> Dict[str, Any]:
        ...
        examples_text = ""
        rag_time = state.get("query_rag_time", 0.0)

        if cfg.enable_query_rag and resources.query_rag_store is not None:
            retrieval_question = (state.get("previous_question") or state.get("question") or "").strip()
            if retrieval_question:
                t0 = time.perf_counter()
                examples = resources.query_rag_store.retrieve(
                    retrieval_question,
                    top_k=cfg.query_rag_top_k,
                )
                rag_time = time.perf_counter() - t0
                examples_text = format_retrieved_examples(examples)

        return {
            "schema": schema,
            "schema_instruction": resources.schema_instruction,
            "cheat_sheet_general": resources.cheat_sheet_general,
            "retrieved_query_examples": examples_text,
            "query_rag_time": rag_time,
        }
...
def build_config_from_env() -> Config:
    ...
    return Config(
        ...
        enable_query_rag=env_bool("ENABLE_QUERY_RAG", True),
        bag_of_queries_path="bag_of_queries.py",
        query_rag_index_dir=".cache/query_rag",
        query_rag_top_k=3,
        query_rag_embedding_model=DEFAULT_EMBEDDING_MODEL,
    )
    ...
    try:
        ...
        return {
            ...
            "timing": {
                ...
                "query_rag_time": final_state.get("query_rag_time", 0.0),
            },
            "artifacts": {
...
def main():
    ...
    print(
        ...
        f"Query-RAG enabled: {cfg.enable_query_rag}, "
    ...
    while True:
        ...
        print("\n--- TIMING ---")
        print(f"Query RAG retrieval:                    {final_state.get('query_rag_time', 0.0):.4f} sec")
