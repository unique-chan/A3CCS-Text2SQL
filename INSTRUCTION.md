...
- 컬럼
  ...
  - `side`: `b` 또는 `op` 값이 저장됨. 아군 (`b`) 및 적군 (`op`)을 구분하기 위한 식별자.
  - `seq`: 같은 스냅샷/팀 내에서 이벤트 배열의 순번(중복 시간 대비용). (Text2SQL 시 꼭 필요한 컬럼은 아니므로, 가급적 사용 제한.)
  - `unit`: `gunner`가 탑승한 차량. (유의사항: unit이라고 변수명은 되어 있으나, vehicle 차량 장비도 저장될 수 있음!) 약 `gunner`와 `unit`이 일치할 경우, `unit`이 차량에서 하차하여, 발사/사격 행위를 실시하였음을 의미. (아닐 경우, 차량에 탑승하여 발사/사격 행위 실시.) 
...
- 예시: event_fired 테이블에 저장된 최상단 레코드는?
    ...
    - 🔍해석: 2026년 3월 3일 14시 8분경, 아군 (b) b_1_m2_2_u2는 b_1_m2_2_v1 차량에 탑승한 뒤, RHS_weap_M242BC 탄약을 이용하여 발사를 시도하였다. (이 발사 행위로 인해 피해를 입은 유닛/차량이 누구인지는 다른 테이블을 조회해야 함.)
